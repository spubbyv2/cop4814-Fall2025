...
class ForecastDisplay(Observer, DisplayElement):
    ...
    def __init__(self, weather_data): #assigned default values for current and last pressure is set to zero pre-update
        self._current_pressure = 29.92
        self._last_pressure = 0.0
        self._weather_data = weather_data

        weather_data.registerObserver(self) # responsible for setting ForecastDisplay as an observer

    def update(self, temp, humidity, pressure):
        self._last_pressure = self._current_pressure # when there is an update, set last_pressure to what *was* current
        self._current_pressure = pressure # assign current_pressure to what it is now after an update
        self.display() # display this change in information

    def display(self):
        print("Forecast: "),
        if self._current_pressure > self._last_pressure: # if the current pressure is increased
            print("Improving weather on the way!")
        elif self._current_pressure == self._last_pressure: # if the current pressure never change from what it was previously
            print("More of the same")
        elif self._current_pressure < self._last_pressure: # if the current pressure is decreased
            print("Watch out for cooler, rainy weather")

        print('***********************************************')

if __name__ == '__main__':
    # TODO: instantiate an object of Subject
